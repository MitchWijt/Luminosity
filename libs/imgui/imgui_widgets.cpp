...
bool ImGui::ImageButtonWithText(ImTextureID texId,const char* label,const ImVec2& imageSize, const ImVec2 &uv0, const ImVec2 &uv1, int frame_padding, const ImVec4 &bg_col, const ImVec4 &tint_col) {
    ...
    const ImVec2 padding = (frame_padding >= 0) ? ImVec2((float)frame_padding * 4, (float)frame_padding) : style.FramePadding;
    ...
    if (size.y<textSize.y)
        start.y += (textSize.y-size.y)*.8f;
    
    const ImRect image_bb(start, start + size);
    
    const ImVec2 sizeWithPadding = size + padding;
    
    start = window->DC.CursorPos;
    start.x += (sizeWithPadding.x / 2) - (textSize.x / 2 * 0.65);
    start.y += size.y * 1.0f;
    ...
    if (textSize.x>0) RenderText(start,label);
    return pressed;
}
